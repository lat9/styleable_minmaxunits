...
// Return a products quantity minimum and units display
 function zen_get_products_quantity_min_units_display($product_id, $include_break = true, $shopping_cart_msg = false) {
    ...
    if ($check_min != 1 or $check_units != 1) {
      if ($check_min != 1) {
        $the_min_units .= '<span class="qmin">' . PRODUCTS_QUANTITY_MIN_TEXT_LISTING . '&nbsp;' . $check_min . '</span>';  //-styleable_minmaxunits-lat9  *** 1 of 6 ***
      }
      if ($check_units != 1) {
        $the_min_units .= '<span class="qunit">' . ($the_min_units ? ' ' : '' ) . PRODUCTS_QUANTITY_UNIT_TEXT_LISTING . '&nbsp;' . $check_units . '</span>';  //-styleable_minmaxunit-lat9  *** 2 of 6 ***
      }
...
// don't check for mixed if not attributes
      ...
      if ($chk_mix != 'none') {
        ...
        $the_min_units .= '</span>';  //-styleable_minmaxunit-lat9  *** 4 of 6 ***
      }
    }
    ...
    if ($check_max != 0) {
      $the_min_units .= '<span class="qmax">';  //-styleable_minmaxunit-lat9  *** 5 of 6 ***
      if ($include_break == true) {
      ...
      $the_min_units .= '</span>';  //-styleable_minmaxunit-lat9  *** 6 of 6 ***
    }
